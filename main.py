...
def get_choice(prompt, choices):
    while True:
        ...
        if value in choices:
            return value
        print("WRONG choice. Allowed values: 1 , 2 , 3")

def sign_up_guest(users):
    ??????

def get_non_empty(users):
    ???

def main_menu(): #i havent do finish i dont understand sht d please
    users = read_file(USERS_RECORD, USERS_HEADERS)
    while True:
        print("\n")
        print("======Main Menu======")
        print("\n")
        print("======Midway Theme Park======")
        print("1. Login 2. Sign up as a GUEST 3. Exit")
        choice = get_choice("Select: ", ["1", "2", "3"])
        if choice == "3":
        ...
        user = find_record(users, 1, username)
        if user is None or user[2] != password:
            print("Login failed. Please check ur username and password.")
            continue
        print("Welcome, " + user[1] + " (" + user[3].replace("_", " ").title() + ").")
        log_event(user[0], "Logged in")
        return user
